...
@torch.no_grad()
def compute_score(model, x, t, k, sigma, tau, dataset_config):
	...
	if tau == 0.0:
		...
		score_rie = metric_tensor(tau, score)
		return score_rie
...
@torch.no_grad()
def compute_log_transition_ratio(model, x, x_hat, t, step_size, k, sigma, tau, dataset_config):
	"""Computes log [ k(x | x_hat) / k(x_hat | x) ]"""
	score_x = compute_score(model, x, t, k, sigma, tau, dataset_config) 
	score_x_hat = compute_score(model, x_hat, t, k, sigma, tau, dataset_config) 
...
@torch.no_grad()
def compute_score_integral(model, x, x_hat, t, k, sigma, tau, dataset_config, n_segments=10):
	...
	score_hat = compute_score(model, r_flat, t, k, sigma, tau, dataset_config).reshape(r.shape[0], -1) 
...
@torch.no_grad()
def compute_correction(model, x, x_hat, t, step_size, k, sigma, tau, dataset_config):
	"""Computes acceptance rate for MALA and returns corrected x"""
	f = compute_score_integral(model, x, x_hat, t, k, sigma, tau, dataset_config)
	log_transition_ratio = compute_log_transition_ratio(model, x, x_hat, t, step_size, k, sigma, tau, dataset_config)
...
def metric_tensor_patch(tau, score_patch):
	...
	print(f"G mean {G.mean().item()} shape {G.shape}")
...
@torch.no_grad()
def sampling(model, dataset_config, method, k=1.0, sigma=1.0, tau = 0.0, step_scale=None, n_langevin_steps=None):
	...
	for t in ts_desc:
		...
		if method in ["DDPM","DDPM-MT"]:
			if method == "DDPM-MT":
				tau = 1.0
				
			score_hat = compute_score(model, x, t, k, sigma, tau, dataset_config)
			noise = torch.randn_like(x)
			x = (x + beta_t * score_hat) / sqrt_alpha_t + sqrt_beta_t * noise

		elif method in ["ULA", "MALA","ULA-MT","MALA-MT"]:

			if method in ["ULA-MT", "MALA-MT"]:
				tau = 1.0
			...
			for langevin_step in range(n_langevin_steps):
				...
				if method in ["ULA","ULA-MT"]:
					x = x_hat
				
				elif method in ["MALA","MALA-MT"]:
					x, a = compute_correction(model, x, x_hat, t, step_size, k, sigma, tau, dataset_config)
